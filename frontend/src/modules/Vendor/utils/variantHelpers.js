...
export const normalizeVariantStateForForm = (rawVariants = {}, basePrice = "", baseWholesalePrice = "") => {
  ...
  const fallbackWholesalePrice = parsePriceValue(baseWholesalePrice);
  ...
  combinations.forEach(({ size, color, key }) => {
    ...
    const resolvedWholesale = resolveCombinationPrice(rawVariants?.wholesalePrices, size, color);
    if (resolvedWholesale !== null) wholesalePrices[key] = resolvedWholesale;
    else if (fallbackWholesalePrice !== null) wholesalePrices[key] = fallbackWholesalePrice;
  ...
  return { sizes, colors, attributes, prices, wholesalePrices, stockMap, imageMap, defaultVariant, defaultSelection };
};
...
export const syncVariantPricesWithAxes = (
  ...
  attributes = [],
  fallbackPrice = "",
  fallbackWholesalePrice = ""
) => {
  const combinations = buildVariantCombinations(sizes, colors, attributes);
  const nextPrices = {};
  const nextWholesalePrices = {};
  const nextStockMap = {};
  const nextImageMap = {};
  const parsedFallback = parsePriceValue(fallbackPrice);
  const parsedFallbackWholesale = parsePriceValue(fallbackWholesalePrice);

  combinations.forEach(({ key }) => {
    const parsedCurrentPrice = parsePriceValue(currentPrices[key]);
    const parsedCurrentWholesalePrice = parsePriceValue(currentWholesalePrices[key]);
    ...
    if (parsedCurrentWholesalePrice !== null) {
      nextWholesalePrices[key] = parsedCurrentWholesalePrice;
    } else if (parsedFallbackWholesale !== null) {
      nextWholesalePrices[key] = parsedFallbackWholesale;
    } else if (parsedFallback !== null) {
      // Fallback to retail price if wholesale is not set
      nextWholesalePrices[key] = parsedFallback;
    }
  ...
  return {
    prices: nextPrices,
    wholesalePrices: nextWholesalePrices,
...
export const buildVariantPayload = (rawVariants = {}) => {
  ...
  combinations.forEach(({ key }) => {
    ...
    const parsedWholesalePrice = parsePriceValue(rawVariants?.wholesalePrices?.[key]);
    if (parsedWholesalePrice !== null) {
      wholesalePrices[key] = parsedWholesalePrice;
    }
  ...
  if (!hasVariants) return { sizes: [], colors: [], attributes: [], prices: {}, wholesalePrices: {}, stockMap: {}, imageMap: {}, defaultVariant: {}, defaultSelection: {} };
