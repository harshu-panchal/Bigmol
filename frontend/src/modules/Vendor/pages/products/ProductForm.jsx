...
const ProductForm = () => {
  ...
  const updateVariantAxes = (axis, rawText) => {
    ...
    const synced = syncVariantPricesWithAxes(
      ...
      formData.variants?.wholesalePrices || {},
  ...
  const handleSubmit = async (e) => {
    ...
    const parsedMinimumOrderQuantity = formData.minimumOrderQuantity
      ...
      : null;
    const parsedWholesalePrice = formData.wholesalePrice
      ? parseFloat(formData.wholesalePrice)
      : null;
    const parsedMinWholesaleQty = parseInt(formData.minWholesaleQty, 10);
              ...
              <div className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                ...
                <div className="space-y-2">
                  {variantCombinations.map((combo) => (
                    <div key={combo.key} className="grid grid-cols-1 md:grid-cols-5 gap-2 items-center">
                      <p className="text-[10px] text-gray-700 font-medium md:col-span-1 truncate">
                      ...
                      <input
                        ...
                        className="w-full px-2 py-1.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-[10px]"
                        placeholder="Retail"
                      />
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                        value={formData.variants?.wholesalePrices?.[combo.key] ?? ""}
                        onChange={(e) => {
                          const nextValue = e.target.value;
                          setFormData((prev) => ({
                            ...prev,
