...
const AnimatedBanner = ({ banners = null }) => {
  ...
  return (
    <div className="px-3 py-1.5">
      <div className="relative w-full h-20 rounded-xl overflow-hidden shadow-md">
            ...
            return (
              <motion.div
                ...
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
                  ...
                  <div className={`absolute right-[5%] top-1/2 -translate-y-1/2 w-20 h-20 flex items-center justify-center ${banner.id === 2 ? 'pb-4' : ''}`}>
                    <motion.div
                ...
                <button
                  ...
                  className="relative z-10 h-full flex pt-1.5 justify-between group">
                  <div className="flex-1">
                    <motion.div
                      ...
                      <motion.div
                        ...
                        <Icon className="text-white text-sm drop-shadow-lg" />
                      </motion.div>
                      <motion.span
                        className="text-white/90 text-[9px] font-medium"
                        animate={{
                          opacity: [0.9, 1, 0.9],
                        }}
                        transition={{
                    ...
                    <motion.h3
                      ...
                      className="text-white text-sm font-extrabold mb-0 drop-shadow-lg relative inline-block">
                      {banner.title}
                    </motion.h3>
                    ...
                    <motion.div
                      ...
                      className="inline-flex items-center gap-1 bg-white/25 px-2 py-1 rounded-full relative overflow-hidden"
                      whileTap={{ scale: 0.95 }}>
                      <span className="text-white font-bold text-[9px] relative z-10">
                        {banner.discount}
                      </span>
                      <FiArrowRight className="text-white text-[9px] relative z-10" />
                    </motion.div>
                  </div>
                </button>
              </motion.div>
        ...
        <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-1 z-20">
          {resolvedBanners.map((_, index) => (
            <button
              ...
              <motion.div
                animate={{
                  width: index === currentBanner ? 14 : 4,
                  opacity: index === currentBanner ? 1 : 0.5,
                }}
                transition={{ duration: 0.3 }}
                className={`h-1 rounded-full bg-white ${index === currentBanner ? "w-3.5" : "w-1"
                  }`}
              />
            </button>
          ))}
