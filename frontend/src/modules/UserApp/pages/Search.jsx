    ...
    <PageTransition>
      <MobileLayout showBottomNav={true} showCartBar={true}>
        <div className="w-full pb-24 lg:pb-12 max-w-7xl mx-auto min-h-screen bg-gray-50">
          {/* Search Header */}
          <div className="px-3 py-2 bg-white border-b border-gray-100 sticky top-1 z-30">
            <form onSubmit={handleSearch} className="mb-2 lg:hidden">
              <div className="relative">
                <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-base z-10" />
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  className="w-full pl-9 pr-16 py-2 glass-card rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 text-gray-700 placeholder:text-gray-400 text-sm"
                  autoFocus
                />
                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    className={`p-1.5 rounded-lg transition-colors ${isListening ? 'bg-red-100 text-red-600' : 'hover:bg-gray-100 text-gray-400'}`}
                  >
                    <motion.div animate={isListening ? { scale: [1, 1.2, 1] } : {}} transition={{ duration: 0.5, repeat: isListening ? Infinity : 0 }}>
                      <FiMic className="text-sm" />
                    </motion.div>
                  </motion.button>
                  {searchQuery && (
                    <button type="button" onClick={() => { setSearchQuery(''); setSearchParams({ sort: sortBy || 'newest' }); setShowSuggestions(false); }} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors text-gray-400">
                      <FiX className="text-sm" />
                    </button>
                  )}
                </div>
                <SearchSuggestions query={searchQuery} isOpen={showSuggestions} onSelect={handleSuggestionSelect} onClose={() => setShowSuggestions(false)} recentSearches={recentSearches} onDeleteRecent={deleteRecentSearch} onClearRecent={clearRecentSearches} />
              </div>
            </form>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                <select
                  ...
                  className="px-2 py-1 text-[10px] rounded-lg border border-gray-200 bg-white text-gray-700 focus:outline-none focus:ring-1 focus:ring-primary-500"
                >
                ...
                <div className="flex items-center bg-gray-100 rounded-lg p-0.5">
                  <button onClick={() => setViewMode('list')} className={`p-1 rounded transition-colors ${viewMode === 'list' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500'}`}>
                    <FiList className="text-sm" />
                  </button>
                  <button onClick={() => setViewMode('grid')} className={`p-1 rounded transition-colors ${viewMode === 'grid' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500'}`}>
                ...
                <div ref={filterButtonRef} className="relative">
                  <button
                    ...
                    className={`flex items-center gap-1 px-2.5 py-1 glass-card rounded-lg hover:bg-white/80 transition-colors ${showFilters ? "bg-white/80" : ""}`}
                  >
                    <FiFilter className={`text-xs transition-colors ${hasActiveFilters ? "text-blue-600" : "text-gray-600"}`} />
                    <span className="font-semibold text-gray-700 text-[10px]">Filter</span>
                  </button>
