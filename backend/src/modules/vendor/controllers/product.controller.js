...
const normalizeVariantsPayload = (rawVariants = {}, fallbackPrice, fallbackWholesalePrice) => {
    if (!rawVariants || typeof rawVariants !== 'object') {
        const resetData = { sizes: [], colors: [], attributes: [], prices: {}, wholesalePrices: {}, stockMap: {}, imageMap: {}, defaultVariant: {}, defaultSelection: {} };
        return resetData;
    }
    ...
    const wholesalePricesSource = Object.fromEntries(toObjectEntries(rawVariants.wholesalePrices));
    ...
    combinations.forEach(({ selection }) => {
        ...
        const rawWholesalePrice = wholesalePricesSource[key];
        const parsedWholesalePrice = Number(rawWholesalePrice);
        if (Number.isFinite(parsedWholesalePrice) && parsedWholesalePrice >= 0) {
            wholesalePrices[key] = parsedWholesalePrice;
        } else {
            const fallbackW = Number(fallbackWholesalePrice);
            if (Number.isFinite(fallbackW) && fallbackW >= 0) {
                wholesalePrices[key] = fallbackW;
            } else {
                // if no wholesale price at all, we might want to default to retail or 0
                // but let's stick to explicitly provided wholesale price or 0
                const fallbackR = Number(fallbackPrice);
                wholesalePrices[key] = Number.isFinite(fallbackR) ? fallbackR : 0;
            }
        }
...
export const createProduct = asyncHandler(async (req, res) => {
    ...
    const wholesalePrice = Number(rest.wholesalePrice || price);
    if (!Number.isFinite(price) || price < 0) {
        throw new ApiError(400, 'Invalid product price.');
    }
    const normalizedVariants = normalizeVariantsPayload(rest.variants, price, wholesalePrice);
...
export const updateProduct = asyncHandler(async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, 'variants')) {
        ...
        product.variants = normalizeVariantsPayload(req.body.variants, retailPrice, wholesalePrice);
