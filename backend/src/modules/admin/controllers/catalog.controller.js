...
const normalizeVariantsPayload = (rawVariants = {}, fallbackPrice, fallbackWholesalePrice) => {
    ...
    combinations.forEach(({ selection }) => {
        ...
        // Wholesale Price
        const wholesaleSource = Object.fromEntries(toObjectEntries(rawVariants.wholesalePrices));
        const parsedWholesale = toNonNegativeNumber(wholesaleSource[key]);
        if (parsedWholesale !== null) {
            wholesalePrices[key] = parsedWholesale;
        } else {
            // Priority 1: Product base wholesalePrice
            // Priority 2: Variant retail price
            // Priority 3: Product base retailPrice
            const fallbackWholesale = toNonNegativeNumber(rawVariants.wholesalePrice); // Passed in rawVariants
            const variantRetail = prices[key];
            if (fallbackWholesale !== null) {
                wholesalePrices[key] = fallbackWholesale;
            } else if (variantRetail !== null) {
                wholesalePrices[key] = variantRetail;
            }
        }
...
export const createProduct = asyncHandler(async (req, res) => {
    ...
    const normalizedVariants = normalizeVariantsPayload(rest.variants, rest.price, rest.wholesalePrice);
...
export const updateProduct = asyncHandler(async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'variants')) {
        ...
        const fallbackWholesalePrice =
            Object.prototype.hasOwnProperty.call(payload, 'wholesalePrice')
                ? payload.wholesalePrice
                : (await Product.findById(req.params.id).select('wholesalePrice').lean())?.wholesalePrice;
        payload.variants = normalizeVariantsPayload(payload.variants, fallbackPrice, fallbackWholesalePrice);
