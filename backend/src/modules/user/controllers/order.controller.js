...
const resolveVariantSelection = (product, selectedVariant, isWholesaler = false) => {
    const basePrice = Number(isWholesaler && product?.wholesalePrice ? product.wholesalePrice : product?.price);
    ...
    const wholesalePriceEntries = toVariantPriceEntries(product?.variants?.wholesalePrices);
    const targetEntries = isWholesaler && wholesalePriceEntries.length ? wholesalePriceEntries : priceEntries;
    ...
    if (hasDynamicAxes) {
        ...
        const exact = targetEntries.find(([rawKey]) => String(rawKey).trim() === selectionKey);
        if (exact) {
    ...
    if (!targetEntries.length || (!size && !color)) {
    ...
    for (const candidate of candidateKeys) {
        const exact = targetEntries.find(([rawKey]) => String(rawKey).trim() === candidate);
        if (exact) {
            const price = Number(exact[1]);
            if (Number.isFinite(price) && price >= 0) {
...
export const placeOrder = asyncHandler(async (req, res) => {
    ...
    const { items, shippingAddress, paymentMethod, couponCode, shippingOption, downPaymentAmount, emiDetails } = req.body;
    
    // Normalize and restrict methods
    const allowedMethods = ['online', 'cod', 'emi'];
    const normalizedPaymentMethod = paymentMethod === 'cash' || paymentMethod === 'COD' ? 'cod' : paymentMethod;
    ...
    const isWholesaler = req.user?.userType === 'wholesaler';
    ...
    if (idempotencyKey) {
        const existingOrder = await Order.findOne({ idempotencyScope, idempotencyKey })
            .select('orderId total trackingNumber razorpayOrderId downPaymentAmount paymentMethod emiDetails')
            .lean();
        if (existingOrder) {
            return res.status(200).json(
                new ApiResponse(
                    200,
                    {
                        ...
                        downPaymentAmount: existingOrder.downPaymentAmount,
                        paymentMethod: existingOrder.paymentMethod,
                        emiDetails: existingOrder.emiDetails,
                        idempotentReplay: true,
                    },
                    'Duplicate order request. Returning existing order details.'
                )
            );
        }
    }
    ...
    for (const item of items) {
        ...
        if (isWholesaler) {
            if (!product.wholesalePrice) {
                throw new ApiError(400, `${product.name} does not have a wholesale price.`);
            }
            if (item.quantity < (product.minWholesaleQty || 1)) {
                throw new ApiError(400, `Minimum quantity for wholesale price on ${product.name} is ${product.minWholesaleQty || 1}.`);
            }
        }
    ...
    // 5. Build EMI Details and Downpayment for Production
    let calculatedDownPayment = 0;
    if (normalizedPaymentMethod === 'emi') {
        const allowedProviders = ['snapmint', 'bajaj', 'tvs'];
        const provider = String(emiDetails?.provider || req.body?.provider || '').toLowerCase().trim();
        
        if (!allowedProviders.includes(provider)) {
            throw new ApiError(400, `Invalid EMI provider: "${provider}". Allowed: ${allowedProviders.join(', ')}`);
        }

        if (provider === 'snapmint') {
            calculatedDownPayment = Math.round(total * 0.25);
        } else if (provider === 'bajaj') {
            calculatedDownPayment = 0; // Bajaj is usually No Cost, 0 downpayment with processing fee
        } else {
            calculatedDownPayment = 0; // Default for others unless specified
        }
    }
    ...
    // Check if session/transaction is supported (Standalone Mongo fix)
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            const [createdOrder] = await Order.create([{
                ...
                emiDetails: emiDetails || undefined,
            ...
            try {
                for (const item of enrichedItems) {
                    ...
                    // Track for potential rollback in non-transactional fallback
                    deductedItems.push({ 
                ...
                // Manual rollback if session doesn't handle it
                throw stockError;
            }
            ...
            if (['card', 'emi', 'upi', 'wallet', 'bank'].includes(normalizedPaymentMethod)) {
                ...
                const razorpayOrder = await razorpayInstance.orders.create({
                    ...
                    notes: {
                        ...
                        isEMI: String(normalizedPaymentMethod === 'emi')
                    }
                });
        ...
        console.error("CRITICAL_ORDER_ERROR:", err);
        
        // Manual rollback helper for non-transactional or failed transactional states
        const rollbackStock = async (itemsList) => {
            if (!itemsList || itemsList.length === 0) return;
            try {
                console.log("Rolling back stock for:", itemsList.length, "items");
                for (const dItem of itemsList) {
                    const vPath = dItem.variantKey ? `variants.stockMap.${dItem.variantKey}` : null;
                    const rollbackPayload = { $inc: { stockQuantity: Number(dItem.quantity) } };
                    if (vPath) rollbackPayload.$inc[vPath] = Number(dItem.quantity);
                    
                    await Product.updateOne({ _id: dItem.productId }, rollbackPayload);
                    // Also reset status to in_stock if it was out_of_stock
                    await Product.updateOne(
                        { _id: dItem.productId, stock: 'out_of_stock' }, 
                        { $set: { stock: 'in_stock' } }
                    );
                }
            } catch (revertErr) {
                console.error("CRITICAL_INVENTORY_RESTORE_FAILURE:", revertErr);
            }
        };

        // Fallback for standalone MongoDB where transactions fail
        if (err.message?.includes('transaction') || err.codeName === 'CommandNotSupported') {
            console.warn("Transactions not supported. Using manual rollback strategy...");
            const deductedItemsForManual = [];
            try {
                ...
                for (const item of enrichedItems) {
                    ...
                    const updProduct = await Product.findOneAndUpdate(
                        { _id: item.productId, stockQuantity: { $gte: Number(item.quantity) } },
                        updPayload,
                        { new: true }
                    );
                    if (!updProduct) throw new ApiError(409, `Stock unavailable for ${item.name}`);
                    deductedItemsForManual.push({ productId: item.productId, quantity: item.quantity, variantKey: item.variantKey });
                }
            } catch (fallbackErr) {
                await rollbackStock(deductedItemsForManual);
                throw fallbackErr;
            }
        } else {
            throw err;
    ...
    res.status(responseStatus).json(
        new ApiResponse(
            responseStatus,
            {
                ...
                emiDetails: order.emiDetails || null,
